...
`--pubkey` 选项指定生成公钥，`--prikey` 选项指定生成私钥，`-t` 选项指定所需的签名数，即 `threshold`。

你会发现，三个命令都生成了相同的多签账户信息：
...
注意：本教程使用 Docker 操作，如果使用其它平台会略有差别。
...
该命令会生成另一个多签交易签名文件，包含有 tom 和 alice 的签名。
