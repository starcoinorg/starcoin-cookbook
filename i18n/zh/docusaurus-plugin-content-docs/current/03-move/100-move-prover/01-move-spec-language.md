...
内置函数位于模块的未命名外部范围内。如果模块定义了一个函数 `len`，那么这个定义将隐藏相应的内置函数。要在这种情况下访问内置函数，可以用 `::len(v)` 这种写法。
...
值得注意的是，可以使用 `old()` 的表达式是有限制的。在没有仔细理解它的意义之前，我们很容易错误地认为所有“值随时间变化的表达式”都可以放在其中。但如果你试卷写下类似下面这样一个函数和 spec：
