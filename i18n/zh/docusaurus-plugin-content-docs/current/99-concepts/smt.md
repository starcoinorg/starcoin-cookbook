...
有 A, B, C, D 四个交易在虚线框内，在 Merkle Tree属于 Data Blocks， 这部分叫做 LeafNode, 上面的虚线框属于Hash Pointer。
...
如果校验交易A是否存在， 这时候是把交易 A 的Hash值这里记为Hash(A)发送给校验放, 校验方发送一个Hash值列表[Hash(B), Hash(CD)]。
...
图1是一个Merkel Tree，图2优化将空子树用PlaceHolder(方格)代替， 节省了空间，
...
这里图中的Merkle Tree的key的长度都是8个bit，有很多空节点，稀疏，可以进行压缩优化。
...
Null就是前面提到的PlaceHolder， Internal最多有16个子节点(子节点类型可以是Internal或者LeafNode， 这里对应一个HashMap， 子节点索引为为0-16)， LeafNode存储的是实际的Key， Value。
区块链中需要保存历史状态，这里如何查询某个key的历史状态，之前提到Merkle Tree里保存Root_Hash就认为是保存了整棵树，
需要提供树的根节点值和查询的Key，这个根节点就是在starcoin中BlockHeader中的state_root， 这也是后续讲到StateTree的构建需要用到state_root。
starcoin中SMT需要持久化到KvStore， 这里用的是RocksDB(测试中MockTreeStore使用的是HashMap + BTreeSet)，
为了将整个SMT保存在KvStore中， SMT的所有节点都只存储Hash值(对应的内容通过KvStore保存和查询)，存储时需要将Null， Internal， LeafNode节点序列化存储在KvStore中。
...
公共前缀0x12， 0x1也需要生成Internal， 这里先构造0x12的Internal记为Children2， Children2[3] = Hash(Children1)。
...
SMT会产生LeafNode和Internal，会把这些按照Hash值和自身存到BTreeMap中，
